...
public class PreparationUserController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "사용자 준비과정 세팅 완료", content = @Content(mediaType = "application/json", schema = @Schema(example = ""))),
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "사용자 준비과정 수정 완료", content = @Content(mediaType = "application/json", schema = @Schema(example = ""))),
            @ApiResponse(responseCode = "4XX", description = "잘못된 요청", content = @Content(mediaType = "application/json", schema = @Schema(example = "실패 메세지(정확히 어떤 메세지인지는 모름)")))
    })
    @PostMapping("/modify")
